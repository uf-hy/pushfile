...
def ensure_legacy_user() -> None:
    ...
    try:
        ...
        if row:
            _ = conn.execute(
                "UPDATE users SET username = ?, password_hash = ?, root_path = ?, is_active = 1, is_legacy = 1 WHERE id = ?",
                ("admin", password_hash, str(BASE_DIR), LEGACY_USER_ID),
            )
        else:
            _ = conn.execute(
                "INSERT INTO users (id, username, password_hash, root_path, is_active, is_legacy, created_at) VALUES (?, ?, ?, ?, 1, 1, ?)",
...
def create_user(username: str, password: str) -> UserRecord:
    ...
    try:
        _ = conn.execute(
            "INSERT INTO users (id, username, password_hash, root_path, is_active, is_legacy, created_at) VALUES (?, ?, ?, ?, 1, 0, ?)",
            (user_id, username, user["password_hash"], user["root_path"], user["created_at"]),
        )
        conn.commit()
    finally:
        conn.close()
