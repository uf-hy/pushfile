document.addEventListener('DOMContentLoaded', () => {
    ...
    function drawPreviewToCanvas(bitmap) {
        ...
        previewMetaEl.textContent = `原图 ${bitmap.width}×${bitmap.height}  |  预览 ${outW}×${outH}  |  ${fmt}${state.transparentBg && fmt === 'PNG' ? '（透明背景）' : ''}`;
    }

    function renderTiles(bitmap) {
        const { rects } = getTileLayout(bitmap);
        tilesGridEl.style.setProperty('--tile-gap', `${clampInt(state.gap, 0, 60)}px`);
        ...
        tilesMetaEl.textContent = '点击“下载 ZIP / 保存到文件夹”会导出原始尺寸切图';
    }
